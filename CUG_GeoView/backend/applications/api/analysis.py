...
# from applications.common.path_global import up_dir, generate_dir, up_url, fun_type_1, fun_type_4, fun_type_5, fun_type_3, \
...
def change_detection_api():
    ...
    try:
        model_info = get_model_info(model_path)
        # Allow both change_detector and segmenter
        m_type = model_info["_Attributes"]["model_type"]
        if m_type != "change_detector" and m_type != "segmenter":
            return fail_api("模型类型不正确，请检查")
    except:
        return fail_api("模型不存在，请检查")
    list_ = req_json["list"]
